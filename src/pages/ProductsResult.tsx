            ...
            {items.length === 0 ? (
              ...
              items.map((item, i, arr) => (
                <Link
                  key={item.id}
                  to={`/items/${item.id}`}
                  className={`hover:bg-gray-e flex justify-center flex-wrap sm:flex-nowrap p-4 transition-colors ease-in-out ${
                    i !== arr.length - 1
                      ? 'border-b border-solid border-gray-e'
                      : ''
                  }`}
                >
                  <img
                    src={item.picture}
                    alt={item.title}
                    className="w-[180px] h-[180px] mr-4"
                  />
                  <div className="grow">
                    <div className="flex flex-col sm:flex-row items-center justify-between mb-8">
                      <span className="text-2xl text-gray-3 flex items-center">
                        $ {item.price.amount}
                        {item.free_shipping && (
                          <img
                            src={vanIcon}
                            alt="van icon"
                            className="h-6 w-6 sm:h-auto sm:w-auto ml-2"
                          />
                        )}
                      </span>
